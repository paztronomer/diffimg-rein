...
if __name__ == "__main__":
    ...
    ax.add_argument("fits_list", help=h1)
    h2 = "Full path of the log to be created. Suggested: logs/{night}/{N.log}"
    ax.add_argument("logfile", help=h2, type=str)
    h3 = "Number of parallel process to be launched (tip: use nproc to see"
    h3 += " how many processor are available). Default: 8"
    ax.add_argument("-n", help=h3, metavar="", default=8, type=int)
    # Parse arguments
    arg = ax.parse_args()
    
    # Initialize
    P = Pool(processes=arg.n)
    # CK = Change_Keys(logname=arg.logfile)
    ini_logfile(logname=arg.logfile)
